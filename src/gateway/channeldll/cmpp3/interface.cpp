...
uint32_t Cmpp3Biz::ui_seq_id = 0;
unsigned char Cmpp3Biz::uc_seq_id = 0;
...
int Cmpp3Biz::init_biz(const char* conf, void* args,channel_conf_t *channel)
{
...
int Cmpp3Biz::is_login_success(int& is_success )
{
 ...
 */
int Cmpp3Biz::is_packet_complete(const char* msg, int len)
{
    ...
    int ret = m_cmpp3.parse_header(msg,len,&header);
    if( ret != 0 )
    {
        return -1;
    }
...
int Cmpp3Biz::channel_rsp(dict* wq,
                          const char* in_msg, 
                          int in_len,
                          char *sid,
                          char* out_msg, 
                          int& out_len)
{
    ...
    //解析报文头
    cmpp_header_t hdr;
    ret = m_cmpp3.parse_header(in_msg,in_len,&hdr);
    if( ret != 0 )
    {
    ...
    {
    case CMPP3_ACTIVE_TEST://心跳请求报文
        {
            ret = handle_active_test_req(&hdr,
                                    in_msg + CMPP3_HEADER_LENGTH,
                                    hdr.TotalLength - CMPP3_HEADER_LENGTH,
                                    out_msg,
                                    out_len);
            break;
        }
    case CMPP3_ACTIVE_TEST_RESP://心跳应答报文
        {
            LOG_INFO("Recv active_test_resp.seq_id[%u]\n",hdr.SequenceId);
            ret = 1; //不需要返回报文
            break;
        }
    case CMPP3_TERMINATE://停止服务请求报文
        {
    ...
    case CMPP3_TERMINATE_RESP: //停止服务应答报文
        {
            LOG_INFO("Recv terminate_resp.seq_id[%u]\n",hdr.SequenceId);
            ret = 1; //不需要返回报文
            break;
        }
	case CMPP3_CONNECT_RESP: //登录应答报文
		{
    ...
    case CMPP3_SUBMIT_RESP: //发送信息应答报文
		{
    ...
    case CMPP3_DELIVER: //状态、上行短信请求报文
		{
...
int Cmpp3Biz::send_msg_req(dict* wq,
                             ...
                             int& out_len)
{
    ...
    bodySubmit.FeeTerminalType = 0x0;
    bodySubmit.DestTerminalType = 0x0;
    ...
    if (uTextLen <= 70)
    {
        ...
        int body_len = m_cmpp3.make_submit_req(buf,bodySubmit);
        hdr.TotalLength = body_len + CMPP3_HEADER_LENGTH;
        m_cmpp3.make_header(&hdr,out_msg,CMPP3_HEADER_LENGTH);

        out_len = hdr.TotalLength;
        memcpy(out_msg + CMPP3_HEADER_LENGTH,buf,body_len);
    }
    else
    {
        //长短信 需要将短信内容拆分成多条
        ...
        for (index = 1; index <= uPkTotal; index++)
        {
            ...
            int body_len = m_cmpp3.make_submit_req(buf,bodySubmit);
            hdr.TotalLength = body_len + CMPP3_HEADER_LENGTH;
            m_cmpp3.make_header(&hdr,out_msg + total_len ,CMPP3_HEADER_LENGTH);

            memcpy(out_msg + total_len + CMPP3_HEADER_LENGTH,buf,body_len);
...
int Cmpp3Biz::channel_login_req(char* out_msg, int& out_len)
{
    ...
    auth_src = NULL;
    //Version
    cmpp_connect.Version = 0x30;
    //Timestamp
    cmpp_connect.Timestamp = atoi(timestamp);

    char buf[MAX_PACKET_LEN];
    int body_len = m_cmpp3.make_connect_req(buf, cmpp_connect);
    if( body_len <= 0 )
    {
...
int Cmpp3Biz::timer_process( dict* wq,
                              char *sid,
                              char* out_msg, 
                              int& out_len )
{
    ...
    out_len = m_cmpp3.make_activeTest_req(out_msg,seq_id);
...
unsigned char Cmpp3Biz::get_uc_seq()
{
    return uc_seq_id++;
}

int Cmpp3Biz::handle_active_test_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len)
{
    //心跳请求报文
    out_len = m_cmpp3.make_activeTest_rsp(out_msg,hdr->SequenceId,0x0);
    return 0;
}

int Cmpp3Biz::handle_terminate_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len)
{
    //拆除连接请求报文
    out_len = m_cmpp3.make_terminal_rsp(out_msg,hdr->SequenceId);
    return 0;
}

int Cmpp3Biz::handle_connect_rsp(cmpp_header_t *hdr,const char* body,int len)
{
    cmpp_body_connect_resp_t rsp;
    int tmp_len = m_cmpp3.parse_connect_rsp(const_cast<char *>(body),rsp);
    if( tmp_len != len )
    {
    ...
    LOG_INFO("Recv connect_resp.seq_id[%u]status[%u]\n",
                                     hdr->SequenceId,
                                     rsp.Status);
...
int Cmpp3Biz::handle_submit_rsp(cmpp_header_t *hdr,const char* body,int len)
{
    cmpp_body_submit_resp_t rsp;
    int tmp_len = m_cmpp3.parse_submit_rsp(const_cast<char *>(body),rsp);
    if( tmp_len != len )
    {
...
int Cmpp3Biz::handle_deliver_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len)
{
    //状态报告、上行短信报文
    cmpp_body_deliver_t req;
    int tmp_len = m_cmpp3.parse_deliver_req(const_cast<char *>(body),req);
    if( tmp_len != len )
    {
    ...
    else // Deliver消息为状态报告
    {
        ...
        LOG_INFO("Recv report message.seq_id[%u]MsgId[%llu]srcphone[%s]destphone[%s]status[%s]\n", 
                            hdr->SequenceId,
                            uMessageid,
                            req.SrcTerminalId,
                            req.DestId,
    ...
    char buf[MAX_PACKET_LEN];
    int body_len = m_cmpp3.make_deliver_rsp(buf, rsp);
    if( body_len <= 0 )
    {
    ...
    //报文头
    cmpp_header_t head;
    head.CommandId = CMPP3_DELIVER_RESP;
    head.SequenceId = hdr->SequenceId;
    head.TotalLength = body_len + CMPP3_HEADER_LENGTH;
    m_cmpp3.make_header(&head,out_msg,CMPP3_HEADER_LENGTH);
...
int Cmpp3Biz::append_response_map(uint32_t seq_id,message_packet_t *req)
{
    message_response_t msgResponse;
...
void Cmpp3Biz::save_message_response( int type,message_response_t *rsp )
{
    if( rsp == NULL ) return;
...
void Cmpp3Biz::save_message_response(uint32_t seq_id,uint32_t result,uint64_t msg_id)
{
...
bool Cmpp3Biz::format_to_json(message_response_t *rsp,string &sRecvJson)
{
...
void Cmpp3Biz::save_message_report(string sMessageid,
                                  string sSrcPhone,
                                  string sDestPhone,
                                  string sRealStatus,
...
void Cmpp3Biz::save_message_uplink(string sSrcPhone,string sDestPhone,string sMessageContent)
{
    // 判断并去除号码前缀
    string src_phone;
    if( sSrcPhone.length() > 11 )
...
void Cmpp3Biz::do_message_response_timeout()
{
    //清理超时未响应的短信
