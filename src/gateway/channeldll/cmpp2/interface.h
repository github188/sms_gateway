...
class Cmpp2Biz: public IChannelBiz
{
public:
    Cmpp2Biz();
    virtual ~Cmpp2Biz();
...
private:
    int handle_active_test_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len);
    int handle_terminate_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len);
    int handle_connect_rsp(cmpp_header_t *hdr,const char* body,int len);
    int handle_submit_rsp(cmpp_header_t *hdr,const char* body,int len);
    int handle_deliver_req(cmpp_header_t *hdr,const char* body,int len,char* out_msg,int& out_len);
    int load_template();
    bool make_template_xml(string code,string content,string msg_content,string &temp_content);
    uint32_t get_ui_seq();
    ...
    void save_message_response(uint32_t seq_id,uint32_t result,uint64_t msg_id);
    ...
    void RemoveSmsSign(string &sMessageContent);
    
private:
    int                                 m_login_success;   //是否登录成功
    Cmpp2                               m_cmpp2;           //协议解析
    map<string,string>                  m_mCmppTemplate;
