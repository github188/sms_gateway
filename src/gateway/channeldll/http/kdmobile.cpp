...
int KdMobileBiz::random = 0;

KdMobileBiz::KdMobileBiz()
{
...
int KdMobileBiz::channel_rsp(dict* wq,
                          const char* in_msg, 
                          int in_len,
                          char *sid,
                          char* out_msg, 
...
int KdMobileBiz::send_msg_req(dict* wq,
                             ...
                             int& out_len)
{
    ...
    sPostContent.append("&mobilelist=");
    sPostContent.append(sPhoneList);
    sPostContent.append("&msg=");
    sPostContent.append(gbk);
    sPostContent.append("&msgid=");
    sPostContent.append(to_string(seq_id));

    string uri = "/pushsms1069/SendMsg_Post.asp";
...
int KdMobileBiz::timer_process( dict* wq,
                              char *sid,
                              char* out_msg, 
                              int& out_len )
{
    ...
    if( random == 0 )
    {
        random = 1;
        //发送获取短信报告
        uri = "/pushsms1069/getreport_new.asp";
        wi.busi_type = 1;
    }
    else
    {
...
int KdMobileBiz::handle_submit_rsp(uint32_t seq_id,const char* body,int len)
{
    string sRespCode;
    string sMessageid;
    string sPostResponse = body;
    // 提取响应body内容
    size_t nHeadBody = sPostResponse.find("<body>");
    size_t nTailBody = sPostResponse.find("</body>");
    if (nHeadBody == string::npos)
    {
        LOG_ERROR("Find body is failure.\n");
        return -1;
    }
    string sRespContent = sPostResponse.substr(nHeadBody + 6, nTailBody - nHeadBody - 6);
    // 去除回车换行符
    sRespCode = sRespContent.substr(2, sRespContent.length() - 4);
...
int KdMobileBiz::handle_report_rsp(uint32_t seq_id,const char* body,int len)
{
    string sPostResponse = body;
    // 提取响应body内容
    size_t nHeadBody = sPostResponse.find("<body>");
    ...
    string sRespContent = sPostResponse.substr(nHeadBody + 6, nTailBody - nHeadBody - 6);
    // 去除回车换行符
    string sRespCode = sRespContent.substr(2, sRespContent.length() - 4);
    if (sRespCode.compare("0") != 0 && 
        sRespCode.compare("-1") != 0 && 
        sRespCode.compare("-2") != 0)
    {
        vector<string> report_vec;
        SplitString(report_vec,sRespCode,";");
...
int KdMobileBiz::handle_uplink_rsp(uint32_t seq_id,const char* body,int len)
{
    string sPostResponse = body;
    // 提取响应body内容
    size_t nHeadBody = sPostResponse.find("<body>");
        ...
        sRespCode.compare("-2") != 0)
    {
        ...
        for (iter = report_vec.begin(); iter != report_vec.end(); iter++)
        {
            ...
            // 获取消息ID
            string sMobilePhone = item_vec[0];
            // 获取手机号码
            string sMessageContent = item_vec[1];
            // 获取短信状态
            string sDestPhone = item_vec[2];

            string utf8;
            ascii_to_utf8(sMessageContent,utf8);

            LOG_INFO("recv uplink rsp.src_phone[%s]dest_phone[%s]\n",
                                        sMobilePhone.c_str(),
                                        sDestPhone.c_str());
            //将上行短信写入redis
            save_message_uplink(sMobilePhone,sDestPhone,utf8);
        }
    }
    return 1;
}
