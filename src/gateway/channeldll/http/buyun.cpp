...
int BuYunBiz::is_need_to_listen(int& need_to_listen,int& listen_port)
{
    if( m_channel == NULL )
    {
...
int BuYunBiz::channel_req(const char* in_msg,int in_len,char* out_msg,int& out_len)
{
    ...
    {
        //解析请求参数
        string sMessageid = http_get_field(body, "msgid");
        string sReportTime = http_get_field(body, "reportTime");
        string sMobilePhone = http_get_field(body, "mobile");
        string sSmsStatus = http_get_field(body, "status");
        ...
        LOG_INFO("recv report rsp.msgid[%s]phone[%s]status[%s]\n",
                                        ...
                                        sSmsStatus.c_str());
        //将状态报告写入redis
        save_message_report(sMessageid,sMobilePhone,"",sSmsStatus,sSmsStatus);
    }
    else if( strcmp(uri, "/smsuplink") == 0  )
    {
        //解析请求参数
        string sMobilePhone = http_get_field(body, "mobile");
        string sMessageContent = http_get_field(body, "msg");
        string sDestPhone = http_get_field(body, "destcode");
...
int BuYunBiz::channel_rsp(dict* wq,
                          const char* in_msg, 
                          int in_len,
                          char *sid,
                          char* out_msg, 
                          int& out_len)
{
    ...
    */
    ...
    {
        LOG_ERROR("Parser response is failure.The response:%s.", content.c_str());
        return -1;
    }
    size_t szStatusBegin = content.find(",");
    string sRespCode = content.substr(szStatusBegin+1, szStatusEnd-szStatusBegin-1);
    string sMessageid;
    ...
    {
        sMessageid = content.substr(szStatusEnd+1, content.length()-szStatusEnd-2);
    }
...
int BuYunBiz::send_msg_req(dict* wq,
...
int BuYunBiz::timer_process( dict* wq,
                              char *sid,
                              char* out_msg, 
                              int& out_len )
{
