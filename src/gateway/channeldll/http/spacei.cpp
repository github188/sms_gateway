...
int SpaceiBiz::is_need_to_listen(int& need_to_listen,int& listen_port)
{
    if( m_channel == NULL )
    {
...
int SpaceiBiz::channel_req(const char* in_msg,int in_len,char* out_msg,int& out_len)
{
    ...
    {
        //解析请求参数
        string sName = http_get_field(body, "name");
        string sReport = http_get_field(body, "report");

        if ( sName.empty() || sReport.empty() )
        {
    ...
    else if( strcmp(uri, "/smsmo") == 0  ) //上行短信
    {
        //解析请求参数
        string sMobilePhone = http_get_field(body, "src");
        string sMessageContent = http_get_field(body, "content");
        string sDestPhone = http_get_field(body, "reference");
        ...
        string gbk;
        utf8_to_ascii(sMessageContent,gbk);
        LOG_INFO("recv uplink rsp.src_phone[%s]dest_phone[%s]\n",
                                        sMobilePhone.c_str(),
                                        sDestPhone.c_str());
        //将上行短信写入redis
        save_message_uplink(sMobilePhone,sDestPhone,gbk);
    }
    else
    {
...
int SpaceiBiz::channel_rsp(dict* wq,
                          const char* in_msg, 
                          int in_len,
                          char *sid,
                          char* out_msg, 
                          int& out_len)
{
    ...
    string content = body;
    size_t szStatusEnd = content.find(":");
    if (szStatusEnd == string::npos)
    {
        LOG_ERROR("Parser response is failure.The response:[%s]\n", content.c_str());
        return -1;
    }
    string sRespCode = content.substr(0, szStatusEnd);
    string sMessageid;
    if (sRespCode.compare("success") != 0)
    {
        ...
        sRespCode = content.substr(szStatusEnd + 1, content.length());
    }
    else
    {
        sMessageid = content.substr(szStatusEnd + 1);
        //转换成统一成功返回码
        sRespCode = "0";
    }
...
int SpaceiBiz::send_msg_req(dict* wq,
                             ...
                             int& out_len)
{
    ...
    string sKeyValue = MD5result;
    sKeyValue.append(sDateTime);
    memset(MD5result,0,sizeof(MD5result));
    my_compute_md5(sKeyValue.c_str(),sKeyValue.length(),MD5result,sizeof(MD5result));

    string sPostContent;
    sPostContent.append("name=");
    sPostContent.append(m_channel->sUserName);
    sPostContent.append("&seed=");
    sPostContent.append(sDateTime);
    sPostContent.append("&key=");
    sPostContent.append(MD5result);
    sPostContent.append("&dest=");
    sPostContent.append(sPhoneList);
    sPostContent.append("&content=");
    sPostContent.append(msg);
    sPostContent.append("&ext=");
    sPostContent.append("&reference=");
    sPostContent.append(packet->sSrcPhone);

    //http://160.19.212.218:8080/eums/utf8/send_strong.do
    string uri;
...
int SpaceiBiz::timer_process( dict* wq,
                              char *sid,
                              char* out_msg, 
                              int& out_len )
{
