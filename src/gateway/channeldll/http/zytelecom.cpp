...
int ZYTelecomBiz::is_need_to_listen(int& need_to_listen,int& listen_port)
{
    if( m_channel == NULL )
    {
...
int ZYTelecomBiz::channel_req(const char* in_msg,int in_len,char* out_msg,int& out_len)
{
    ...
    {
        //解析请求参数
        string sReport = http_get_field(body, "data");
        if ( sReport.empty() )
        {
    ...
    {
        //解析请求参数
        string sMobilePhone = http_get_field(body, "mb");
        string sMessageContent = http_get_field(body, "ms");
        string sDestPhone = http_get_field(body, "sc");
...
int ZYTelecomBiz::channel_rsp(dict* wq,
                          const char* in_msg, 
                          int in_len,
                          char *sid,
                          char* out_msg, 
...
int ZYTelecomBiz::send_msg_req(dict* wq,
                             ...
                             int& out_len)
{
    ...
    string sPostContent;
    sPostContent.append("&ua=");    // 只需要用户ID不需要用户名,文档使用uid实际需要使用ua
	sPostContent.append(m_channel->sUserName);
	sPostContent.append("&pw=");
	sPostContent.append(MD5result);
	sPostContent.append("&mb=");
	sPostContent.append(sPhoneList);
	sPostContent.append("&ms=");
	sPostContent.append(msg);
	sPostContent.append("&tm=");
...
int ZYTelecomBiz::timer_process( dict* wq,
                              char *sid,
                              char* out_msg, 
                              int& out_len )
{
