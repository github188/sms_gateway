...
int is_packet_complete(const char* buf, unsigned len)
{
    ...
    int ret = g_smgp.parse_header(buf, len, &header);
    if( ret != 0 )
    {
        return -1;
    }

    if( len < header.PacketLength )
    {
        return 0;
    }
...
int handle_packet( connection_t *con )
{
    ...
    // 3. 解析报文头
    smgp_header_t hdr;
    if (g_smgp.parse_header(readptr, datalen, &hdr) != 0)
    {
    ...
    memcpy(buff, readptr, hdr.PacketLength);
    rcvbuf->set_read_size(rcvbuf, hdr.PacketLength);
...
int handle_heartbeat_req(smgp_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    ...
    int packet_len = g_smgp.make_activeTest_rsp(writeptr,hdr->SequenceId,0x00);
...
int handle_svr_reg_req(smgp_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    ...
    int body_len = g_smgp.make_connect_rsp(outbuf, rsp);
    if( body_len <= 0 )
    {
    ...
    //报文头
    smgp_header_t head;
    head.RequestId = CMPP_CONNECT_RESP;
    head.SequenceId = hdr->SequenceId;
    head.PacketLength = body_len + SMGP_HEADER_LENGTH;
    g_smgp.make_header(&head,writeptr,SMGP_HEADER_LENGTH);

    //拷贝报文体
    memcpy(writeptr + SMGP_HEADER_LENGTH, outbuf, body_len);

    //设置缓冲区游标
    pbuffer->set_write_size(pbuffer, head.PacketLength);
...
int handle_gateway_msg_send_req(smgp_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    ...
    int tmp_len = g_smgp.parse_submit_req(const_cast<char *>(data),req);
    if( tmp_len != len )
    {
    ...
    char outbuf[MAX_PACKET_LEN];
    int body_len = g_smgp.make_submit_rsp(outbuf, rsp);
    if( body_len <= 0 )
    {
    ...
    //报文头
    smgp_header_t head;
    head.RequestId = CMPP_SUBMIT_RESP;
    head.SequenceId = hdr->SequenceId;
    head.PacketLength = body_len + SMGP_HEADER_LENGTH;
    g_smgp.make_header(&head,writeptr,SMGP_HEADER_LENGTH);
...
int handle_gateway_report_rsp(smgp_header_t* hdr, const char* data, int len)
{
    ...
    int tmp_len = g_smgp.parse_deliver_rsp(const_cast<char *>(data),rsp);
    if( tmp_len != len )
    {
