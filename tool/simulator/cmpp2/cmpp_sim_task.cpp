...
int time_task(dlist_t *write)
{
    ...
    list_for_each(item, g_phonelist->head, g_phonelist->tail)
    {
        ...
        //报文体
        cmpp2_body_deliver_t req;

        //报文体赋值
        req.MsgId = atol(pInfo->msg_id.c_str());
        req.RegisteredDelivery = 0x01;
        strncpy(req.DestId,pInfo->src_phone.c_str(),sizeof(req.DestId));
        strncpy(req.SrcTerminalId,pInfo->dest_phone.c_str(),sizeof(req.SrcTerminalId));

        req.deliverMessage.MsgId = atol(pInfo->msg_id.c_str());
        strcpy(req.deliverMessage.Stat,"DELIVRD");

        //报文体序列化
        int body_len = g_cmpp2.make_deliver_req(outbuf, req);
        if( body_len <= 0 )
        {
        ...
        g_cmpp2.make_header(&hdr,writeptr,CMPP2_HEADER_LENGTH);
        //拷贝报文体
