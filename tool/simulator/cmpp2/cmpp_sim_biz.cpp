...
int is_packet_complete(const char* buf, unsigned len)
{
    ...
    int ret = g_cmpp2.parse_header(buf, len, &header);
    if( ret != 0 )
    {
        return -1;
    }
...
int handle_packet( connection_t *con )
{
    ...
    // 3. 解析报文头
    cmpp2_header_t hdr;
    if (g_cmpp2.parse_header(readptr, datalen, &hdr) != 0)
    {
...
// 处理心跳请求包
int handle_heartbeat_req(cmpp2_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    if( g_cnt > 1 )
    {
    ...
    int packet_len = g_cmpp2.make_activeTest_rsp(writeptr,hdr->SequenceId,0x00);
...
//处理注册请求包
int handle_svr_reg_req(cmpp2_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    ...
    //返回应答报文
    cmpp2_body_connect_resp_t rsp;
    rsp.Status = 0x0;

    char outbuf[MAX_PACKET_LEN];
    int body_len = g_cmpp2.make_connect_rsp(outbuf, rsp);
    if( body_len <= 0 )
    {
        LOG_ERROR("make_connect_rsp fail.\n");
        return -1;
    }
    if( ( body_len + CMPP2_HEADER_LENGTH ) > freesize )
    {
        return -1;
    }
    //报文头
    ...
    g_cmpp2.make_header(&head,writeptr,CMPP2_HEADER_LENGTH);

    //拷贝报文体
    memcpy(writeptr + CMPP2_HEADER_LENGTH, outbuf, body_len);
...
//信息发送请求
int handle_gateway_msg_send_req(cmpp2_header_t* hdr, const char* data, int len, connection_t *pcon)
{
    cmpp2_body_submit_t req;

    int tmp_len = g_cmpp2.parse_submit_req(const_cast<char *>(data),req);
    if( tmp_len != len )
    {
    ...
    snprintf(msgid,sizeof(msgid),"%s%d%04u",date_time,g_pid,g_seq);

    for( int i = 0 ; i < req.DestUsrtl; i++ )
    {
        char msgid_tmp[64] = {0};
        snprintf(msgid_tmp,sizeof(msgid_tmp),"%s%d%04u",date_time,g_pid,g_seq);
    ...
    //返回应答报文
    cmpp2_body_submit_resp_t rsp;
    rsp.MsgId = atol(msgid);
    rsp.Result = 0x0;

    char outbuf[MAX_PACKET_LEN];
    int body_len = g_cmpp2.make_submit_rsp(outbuf, rsp);
    if( body_len <= 0 )
    {
...
//信息发送应答
int handle_gateway_report_rsp(cmpp2_header_t* hdr, const char* data, int len)
{
    cmpp2_body_deliver_resp_t rsp;

    int tmp_len = g_cmpp2.parse_deliver_rsp(const_cast<char *>(data),rsp);
    if( tmp_len != len )
    {
