...
uint32_t Cmpp2::get_header_len()
{
    return CMPP2_HEADER_LENGTH;
}

int Cmpp2::parse_header(const char* buf, int len, cmpp2_header_t* header)
{
...
int Cmpp2::make_header(cmpp2_header_t* header, char* buf, int len)
{
    if ( buf == NULL || header == NULL )
    {
        return -1;
...
int Cmpp2::make_connect_req(char *buf, cmpp2_body_connect_t body)
{
...
int Cmpp2::make_connect_rsp(char *buf, cmpp2_body_connect_resp_t body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::parse_connect_req(char *buf, cmpp2_body_connect_t & body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::parse_connect_rsp(char *buf, cmpp2_body_connect_resp_t &body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;

    // 解析消息体
    GetBufferChar(ptr, body.Status);
...
int Cmpp2::make_submit_req(char *buf, cmpp2_body_submit_t body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::make_submit_rsp(char *buf, cmpp2_body_submit_resp_t body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::parse_submit_req(char *buf, cmpp2_body_submit_t &body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::parse_submit_rsp(char *buf, cmpp2_body_submit_resp_t &body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::make_deliver_req(char *buf, cmpp2_body_deliver_t body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
    ...
    SetBufferString(ptr, body.Reserved, sizeof(body.Reserved) - 1);

    return (ptr - buf);
}

int Cmpp2::make_deliver_rsp(char *buf, cmpp2_body_deliver_resp_t body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::parse_deliver_req(char *buf, cmpp2_body_deliver_t &body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
    ...
    GetBufferString(ptr, body.Reserved, sizeof(body.Reserved));

    return (ptr - buf);
}

int Cmpp2::parse_deliver_rsp(char *buf, cmpp2_body_deliver_resp_t &body)
{
    if (buf == NULL)  return -1;
    char *ptr = buf;
...
int Cmpp2::make_terminal_req(char *buf,uint32_t seq_id)
{
    cmpp2_header_t header;
    header.TotalLength = CMPP2_HEADER_LENGTH;
    header.CommandId = CMPP_TERMINATE;
    header.SequenceId = seq_id;

    make_header(&header,buf,CMPP2_HEADER_LENGTH);

    return CMPP2_HEADER_LENGTH;
}

int Cmpp2::make_terminal_rsp(char *buf,uint32_t seq_id)
{
    cmpp2_header_t header;
    header.TotalLength = CMPP2_HEADER_LENGTH;
    header.CommandId = CMPP_TERMINATE_RESP;
    header.SequenceId = seq_id;
...
int Cmpp2::make_activeTest_req( char *buf,uint32_t seq_id )
{
    cmpp2_header_t header;
    header.TotalLength = CMPP2_HEADER_LENGTH;
    header.CommandId = CMPP_ACTIVE_TEST;
    header.SequenceId = seq_id;
...
int Cmpp2::make_activeTest_rsp(char *buf,uint32_t seq_id,unsigned char Reserved)
{
    cmpp2_header_t header;
    header.TotalLength = CMPP2_HEADER_LENGTH + sizeof(unsigned char);
    header.CommandId = CMPP_ACTIVE_TEST_RESP;
    header.SequenceId = seq_id;

    make_header(&header,buf,CMPP2_HEADER_LENGTH);

    memcpy(buf + CMPP2_HEADER_LENGTH, &Reserved, sizeof(unsigned char));
