...
//注册请求
typedef struct cmpp2_body_connect_s
{
...
} cmpp2_body_connect_t;

//注册应答
typedef struct cmpp2_body_connect_resp_s
{
...
} cmpp2_body_connect_resp_t;

//短信发送请求
typedef struct cmpp2_body_submit_s
{
    uint64_t MsgId;                // 信息标识
...
} cmpp2_deliver_message_t;

//状态推送，上行短信请求
typedef struct cmpp2_body_deliver_s
{
    uint64_t MsgId;                // 信息标识
    ...
    cmpp2_deliver_message_t deliverMessage;   // 当为RegisteredDelivery为1时，取该内容
    char Reserved[8 + 1];                  // 保留字段
} cmpp2_body_deliver_t;

//状态推送，上行短信应答
typedef struct cmpp2_body_deliver_resp_s
{
    uint64_t MsgId;                // 信息标识
    unsigned char Result;           // 0：正确 1：消息结构错 2：命令字错 3：消息序号重复 4：消息长度错 5：资费代码错 6：超过最大信息长 7：业务代码错 8: 流量控制错 9~ ：其他错误
} cmpp2_body_deliver_resp_t;
...
public:
    ...
    int parse_header(const char* buf, int len, cmpp2_header_t* header);
    int make_header(cmpp2_header_t* header, char* buf, int len);

    int make_connect_req(char *buf, cmpp2_body_connect_t body);
    int make_connect_rsp(char *buf, cmpp2_body_connect_resp_t body);
    int parse_connect_req(char *buf, cmpp2_body_connect_t & body);
    int parse_connect_rsp(char *buf, cmpp2_body_connect_resp_t &body);
    
    int make_submit_req(char *buf, cmpp2_body_submit_t body);
    int make_submit_rsp(char *buf, cmpp2_body_submit_resp_t body);
    int parse_submit_req(char *buf, cmpp2_body_submit_t &body);
    int parse_submit_rsp(char *buf, cmpp2_body_submit_resp_t &body);
    
    int make_deliver_req(char *buf, cmpp2_body_deliver_t body);
    int make_deliver_rsp(char *buf, cmpp2_body_deliver_resp_t body);
    int parse_deliver_req(char *buf, cmpp2_body_deliver_t &body);
    int parse_deliver_rsp(char *buf, cmpp2_body_deliver_resp_t &body);
