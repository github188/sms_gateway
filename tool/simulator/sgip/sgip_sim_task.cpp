...
int time_task(dlist_t *write)
{
    ...
    list_for_each(item, g_phonelist->head, g_phonelist->tail)
    {
        ...
        //报文体赋值
        req.SubmitSequenceNumber[0] = atol(pInfo->msg_id.c_str());
        req.SubmitSequenceNumber[1] = atol(pInfo->msg_id.c_str());
        req.SubmitSequenceNumber[2] = atol(pInfo->msg_id.c_str());
        strncpy(req.DestId,pInfo->src_phone.c_str(),sizeof(req.DestId));
        strncpy(req.UserNumber,pInfo->dest_phone.c_str(),sizeof(req.UserNumber));

        req.State = 0x00;

        //报文体序列化
        int body_len = g_sgip.make_deliver_req(outbuf, req);
        if( body_len <= 0 )
        {
        ...
        //报文头
        cmpp_header_t hdr;
        hdr.CommandId = CMPP_DELIVER;
        hdr.SequenceId = g_cnt;
        hdr.TotalLength = body_len + SGIP_HEADER_LENGTH;

        if( freesize < (int)hdr.TotalLength )
        {
            break;
        }

        g_sgip.make_header(&hdr,writeptr,SGIP_HEADER_LENGTH);
        //拷贝报文体
        memcpy(writeptr + SGIP_HEADER_LENGTH, outbuf, body_len);

        //设置缓冲区游标
        pbuffer->set_write_size(pbuffer, hdr.TotalLength);
